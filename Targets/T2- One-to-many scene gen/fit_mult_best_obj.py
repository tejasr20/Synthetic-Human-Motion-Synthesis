...
if __name__ == "__main__":
    ...
    parser.add_argument("--change", default=False, type=int)
    parser.add_argument("--orient", default=False, type=int)
    parser.add_argument("--input_probability",
                        action='store_true', 
                        default=False)
    # parser.add_argument("--num_scene",
    #                     action='store_true', 
    #                     default=False, help= "Add number of scenes to be generated, less than equal to 4")
    # Assuming that if this is called then we WILL give contact vertices with probability 
    ...
    print("CONTACT LABELS", contact_labels.shape)
    # contact_labels = np.argmax(contact_labels, axis=-1) # takes the maximum: again nullifying the effect. 
		# Calculate the second maximum along the last axis
    # if args.input_probability:
    #     second_max_values = np.partition(contact_labels, -2, axis=-1)[:,:,-2]
    #     print(second_max_values.shape)
    #     check= np.argmax(contact_labels, axis=-1) 
    #     mask = second_max_values > 0
    #     # Calculate the new labels based on the conditions
    #     new_labels = np.where(mask, np.argmax(contact_labels, axis=-1), np.argmax(contact_labels, axis=-1))
    #     # Calculate the percentage of values that were changed
    #     percentage_changed = np.mean(mask) * 100
    #     print("Percentage of values changed: {:.2f}%".format(percentage_changed))
    #     contact_labels = new_labels
    #     # check= np.argmax(contact_labels, axis=-1) 
    check= np.argmax(contact_labels, axis=-1) 
    if args.input_probability & args.change:
        second_max_args = np.argpartition(contact_labels, -2, axis=-1)[:, :, -2]
        second_max_probs = np.partition(contact_labels, -2, axis=-1)[:, :, -2]
        threshold=0.1
        max_args = np.argmax(contact_labels, axis=-1)
        mask = (second_max_probs > threshold) & ((max_args != 0) & (max_args != 2))
        new_labels = np.where(mask, second_max_args, np.argmax(contact_labels, axis=-1))
        percentage_changed = np.mean(mask) * 100
        print("Percentage of values changed: {:.2f}%".format(percentage_changed))
        contact_labels = new_labels
    elif args.input_probability:
        contact_labels= np.argmax(contact_labels, axis=-1) 
    # check = np.argmax(contact_labels, axis=-1)
    if(np.all(contact_labels==check)):
        print("Not actually changing anything")
    # print("INitial ", contact_labels[100])
    ...
    for frame in range(contact_labels.shape[0]):
        ...
        # print("FRAME", frame)
        # if(frame==vertices.shape[0]):
        #     break
        vertices_down.append(vertices[frame * 8]) # change the number here for jump step?
    vertices = np.array(vertices_down)
    contact_labels = np.array(contact_labels_mapped)
    # print("After ", contact_labels[100])
	...
	# coninuing with local majority voting 
    voted_vertices = []
    voted_labels = []
    for label in range(max_label + 1):
        ...
        if len(cluster_points) < cluster_min_points:
            continue
        bin_counts = np.bincount(cluster_contact_labels[cluster_labels == label])
        print("Shape of bin:", bin_counts.shape)
        print("Bin counts:", bin_counts)
        majority_label = np.argmax(bin_counts)
        if len(bin_counts) > 1:
            second_majority_label = np.argsort(bin_counts)[-2]  # Find the second maximum label
            if bin_counts[second_majority_label] >= 10:
                print("\tCluster", label, "has", len(cluster_points), "points")
                print("\tMajority label:", majority_label, label_names[majority_label])
                print("\tSecond majority label:", second_majority_label, label_names[second_majority_label])
                # voted_labels_cluster = np.where(cluster_contact_labels[cluster_labels == label] == majority_label, majority_label, second_majority_label)
                voted_labels_cluster = np.full(cluster_points.shape[0], second_majority_label)
            else:
                print("\tCluster", label, "has", len(cluster_points), "points with majority label of", majority_label, label_names[majority_label])
                voted_labels_cluster = np.full(cluster_points.shape[0], majority_label)
        else: 
            print("\tCluster", label, "has", len(cluster_points), "points with majority label of", majority_label, label_names[majority_label])
            voted_labels_cluster = np.full(cluster_points.shape[0], majority_label)
        # print("shape of bin ",np.bincount(cluster_contact_labels[cluster_labels == label]) )
        # majority_label = np.argmax(np.bincount(cluster_contact_labels[cluster_labels == label]))
        # print("Cluster", label, "has", len(cluster_points), "points with majority label of", majority_label, label_names[majority_label])
        voted_vertices.extend(cluster_points)
        # voted_labels.extend(np.full(cluster_points.shape[0], second_majority_label))
    ...
    voted_labels = np.expand_dims(voted_labels, axis=0)
    vertices = voted_vertices
    contact_labels = voted_labels # Contact labels is set to voted_labels
    print("Voted shapes ", voted_vertices.shape, voted_labels.shape)
    print()
    print()
    ...
    for i, obj_c in enumerate(clusters_classes):
        ...
        best_obj_id = ""
        best_obj_ids = []  # List to store the IDs of the two best object
        # best_obj_loss = float('inf')
        second_best_obj_loss = float('inf')
        second_best_obj_id = ""
        ...
        for obj_dir in os.listdir(obj_class_path):
            ...
            #     best_obj_id = obj_dir
            if best_loss < best_obj_loss:
                second_best_obj_loss = best_obj_loss
                second_best_obj_id = best_obj_id
                best_obj_loss = best_loss
                best_obj_id = obj_dir
            elif best_loss < second_best_obj_loss:
                second_best_obj_loss = best_loss
                second_best_obj_id = obj_dir
        
        print("Best fitted object has ID", best_obj_id)
        print("Second best fitted object has ID", second_best_obj_id)
        json_dict = {}
        json_dict["best_obj_id"] = best_obj_id
        second_json_dict={}
        second_json_dict["best_obj_id"] = second_best_obj_id
        # print("")
        if(args.orient):
            json.dump(second_json_dict, open(os.path.join(cluster_base_path, "best_obj_id.json"), 'w'))
        else:
