...
#     19, # stool
# ])
# The term "mpcat40" refers to the Material Point Categories dataset containing 40 material categories. It is a collection of labels and corresponding color codes representing different materials. The function read_mpcat40 reads this dataset from a TSV (Tab-Separated Values) file.

# The dataset provides a set of 40 material categories along with their corresponding RGB color codes. Each material category has a label name, such as "wood", "glass", "metal", etc., and an associated color code in the form of a hexadecimal (hex) value.
    ...
    if os.path.exists(sdf_path) and os.path.exists(json_path):
        ...
        #The size of the generated SDF would depend on the chosen grid resolution or grid dimensions. In the provided code snippet, the default grid dimensions are set to 256, resulting in an SDF grid of size 256 x 256 x 256. 
		# 3D space is discretized into a grid with 256 divisions along each axis, resulting in a total of 16,777,216 grid cells.
		# The SDF is calculated for each point in the 3D grid defined by the grid dimensions. Each grid cell represents a small volume element in 3D space. For each grid cell, the SDF value is computed as the signed distance to the nearest surface of the concatenated human mesh. The sign of the distance indicates whether the point is inside (-) or outside (+) the mesh, while the magnitude represents the distance to the surface.
		# By generating SDFs for a sequence of concatenated human meshes over time, you can capture the temporal evolution of the combined shape
    else:
    ...
    print()
    print()
    # print("SDF, centroid, extents: ",sdf.shape, centroid.shape, extents.shape)
    # SDF, centroid, extents:  torch.Size([256, 256, 256]) torch.Size([3]) torch.Size([3])
...
# }
    ...
    for label in range(max_label + 1):
        ...
        # Choosing to ignore the minority label 
    ...
    for obj_c in classes_eps:
        ...
        for frame in range(num_frames):
            ...
            # collect all vertices of all clusters of a particular class.
            # So think about this: the bottom part of a chair, earlier has like 5 bed points. you set that to chair
            # in the LMV. Similarly, in the vertical cluster, you perform LMV. Now collect these two clusters 
            # and make a chair. Now if you have multiple chairs then it still works because we recluster 
            # after collecting by contact label. We just want to consolidate our objects now. 
        ...
        # print("CLUSTER LABEL SHAPE ", cluster_#labels.shape)
		# CLUSTER LABEL SHAPE  (358,) (157, ).. 
        max_label = cluster_labels.max()
        print("Clustering took {0} seconds".format(time.time()-start_time))
        print("Num clusters", max_label + 1)
        for label in range(max_label + 1):
        ...
        print()
    print()
    print("Cluster classes, Cluster points, Object_indices", len(clusters_classes), len(clusters_points) ,len(objects_indices))
    # Cluster classes, Cluster points, Object_indices 2 2 2: MPH112_ ..
	# Object class chair Object index 0 Num points 358
    ...
    for i, obj_c in enumerate(clusters_classes):
        ...
        for obj_dir in os.listdir(obj_class_path): 
            # Loks at all possibilities from the 3D fiture dataset
            print("This is looping ", i)
            i+=1
            ...
            # Apply a transformation to the aligned object to move it to the cluster centroid.
